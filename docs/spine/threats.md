...
| The registrar / DNS / GitHub account is taken over | a stranger with a stolen password | **Decided:** two-factor authentication on GitHub, Cloudflare and the registrar (Cloudflare) — the human's item before "go". Checked by `gh api user .two_factor_authentication` for GitHub. |
| Response headers we cannot set (HSTS preload, CSP) | a network attacker; an injected script | **Accepted, because** GitHub Pages sets HSTS on the custom domain (`max-age` one year) and allows no custom headers; a CSP naming script sources cannot be set. Exposure named: an injected inline script would run — the only scripts are ours (built, escaped) and the declared beacon; a `<meta http-equiv="Content-Security-Policy">` is the fallback if ever needed (decision 6 names Cloudflare Pages as the move if headers matter). |
...
- **Accounts:** 2FA on GitHub, Cloudflare — human's item, verified for GitHub by the API.
- **Headers:** HSTS from Pages; CSP not settable — accepted, exposure named above.
