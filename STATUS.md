...
deploying. v1 is stamped when the blockers are cleared on the live
...
identity, favicon C, `NOTES.md` as pre-history. The deploy is running; the
two-axis code review runs in parallel on the committed diff (Standards and
Spec reviewers), because the fixes were already live-blocking. When the deploy
lands I re-read the four blockers on https://permitrulebook.com; when the
reviews return, their findings get one more round if needed. Then v1 waits
