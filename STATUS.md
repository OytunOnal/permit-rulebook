...
**The road is drawn** (2026-09-17, `docs/spine/road.html` from
...
and never fails the deploy. *When you like:* Bing Webmaster Tools → IndexNow
should list today's submission within a day.

**s23 is live** (2026-09-17, deploy `e12ec3c` green, read on the live host:
...
- [x] **s32 reviewed, amended and merged** (2026-09-18, your word). Live.
...
the threat model, the architecture, the critiques, the announcement).**s32 is live** (2026-09-18; data `295f204` then site `f7832f0`, deploy green;
read on the live host: the Researcher card *asked* with the hosting
agreement, the ICT card's six months under *not checked here*, the route
H1s without *§ = section*). v1.2's fixes: **one left — data #17** (the
browser-read strategy for the IND), plus the steward items filed on the way
(Back on a restored record; an unknown link's first paint). The stamp comes
when #17 lands — or when you say the steward items are not v1.2's.
...
steward items filed on the way (Back on a restored record; an unknown link's
first paint). The stamp comes when the last fix lands.
...
four-country headline, P8 and the learn links done; next the Algerian
notice's label, P10, the hold state, `field` on precondition statements,
data #15/#13/#17.
...
dated:* beneath). v1.2's fixes: F2, the four-country headline and P8 done;
next the learn links (your walk), P10, the hold state, `field` on
precondition statements, data #15/#13/#17.
